...
function sellPosition(symbol){
  ...
  const raw=prompt("Колко акции от "+symbol+" продаваш?\\n\\nИмаш общо: "+totalQuantity+" акции.",String(totalQuantity));
  if(raw===null) return;
  const quantity=Number(String(raw).replace(",","."));
  if(!Number.isFinite(quantity)||quantity<=0){ alert("Въведи валидно количество акции."); return; }
  if(quantity>totalQuantity){ alert("Не можеш да продадеш "+quantity+" акции. Имаш само "+totalQuantity+"."); return; }
  const remainingQuantity=totalQuantity-quantity;
  if(!confirm("Потвърди продажбата на "+quantity+" акции от "+symbol+"?\\n\\nОбщо преди продажбата: "+totalQuantity+" акции.\\nЩе останат: "+remainingQuantity+" акции.")) return;
  let toSell=quantity;
  const updatedEntries=[];
  for(const entry of entries){
    const entryQuantity=Number(entry.quantity||0);
    if(toSell<=0){ updatedEntries.push(entry); continue; }
    const soldFromEntry=Math.min(entryQuantity,toSell);
    const leftFromEntry=entryQuantity-soldFromEntry;
    toSell-=soldFromEntry;
    if(leftFromEntry>0) updatedEntries.push({...entry,quantity:leftFromEntry});
  }
  if(remainingQuantity<=0) delete state.positions[symbol];
  else state.positions[symbol]=updatedEntries;
  save();
  logActivity("settings","Position partially/fully sold",{symbol,soldQuantity:quantity,remainingQuantity,entriesBefore:entries,entriesAfter:remainingQuantity>0?updatedEntries:[]});
  renderCards();
}
...
function card(x,target,levels){
  ...
  const positionSummary=owned
    ? '<div class="position">Покупки: <b>'+entries.length+'</b> · Общо акции: <b>'+totalQuantity+'</b> · Средна претеглена входна цена: <b>
...
renderMarketRegime();
renderCards();
+num(avgEntry)+'</b> · Печалба/загуба: <b>'+pnl.toFixed(2)+'%</b></div>'
    : (buy.level!==null&&Number.isFinite(Number(x.high60))
      ? '<div class="position">Предполагаем вход: <b>
  let buySignal="⚪ WAIT",buyReason=buy.level!==null?"Следващо/активно ниво за допокупка: -"+buy.level+"%.":"Няма активно ниво за допокупка.";
  if(buy.kind==="entry"){buySignal="🟢 ENTRY ZONE";buyReason="Достигнато ниво за допокупка: -"+buy.level+"% спрямо 60-дневния връх.";}
  const buySectionTitle=owned?"Допокупка / Продажба":"Покупка";
  const finviz=finvizSummary(x.symbol),trend=stockTrend(x),action=stockAction(x,target,levels),risk=riskAnalysis(x,target,levels);
...
renderMarketRegime();
renderCards();
+num(x.high60*(1-buy.level/100))+'</b> · ниво -'+buy.level+'%</div>'
      : '');
  let buySignal="⚪ WAIT",buyReason=buy.level!==null?"Следващо/активно ниво за допокупка: -"+buy.level+"%.":"Няма активно ниво за допокупка.";
  if(buy.kind==="entry"){buySignal="🟢 ENTRY ZONE";buyReason="Достигнато ниво за допокупка: -"+buy.level+"% спрямо 60-дневния връх.";}
  const buySectionTitle=owned?"Допокупка / Продажба":"Покупка";
