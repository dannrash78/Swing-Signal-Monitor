...
$("stockForm").onsubmit = e => {
  ...
  if ($("holding").checked && Number($("entryPrice").value) > 0) {
    state.positions[s] = Number($("entryPrice").value);
  } else {
...
async function render(){
  ...
  if(!backend){
    cards.innerHTML = `<div class="card"><b>Въведи Backend URL.</b><p>След като публикуваш Cloudflare Worker, постави неговия адрес горе.</p></div>`;
    return;
  }
  ...
  try{
    ...
    cards.innerHTML = `<div class="card error"><b>Backend error</b><p>${escapeHtml(e.message)}</p><p>Провери URL, CORS и дали Worker е публикуван.</p></div>`;
  }
}
...
function card(x,target,levels){
  ...
  if(entry){
    ...
    if(pnl >= target){ cls="exit"; signal="🔵 EXIT ZONE"; reason=`Позицията е на ${pnl.toFixed(2)}% спрямо входа. Целта +${target}% е достигната.`; }
    else { cls="watch"; signal="🟡 HOLD / WATCH"; reason=`Позицията е на ${pnl.toFixed(2)}% спрямо входа. Целта е +${target}%.`; }
  } else if(reached){
    cls="entry"; signal="🟢 ENTRY ZONE"; reason=`Цената е ${Math.abs(dd).toFixed(2)}% под 60-дневния връх. Достигнато ниво: -${reached}%.`;
  } else {
    const next=levels.slice().sort((a,b)=>a-b).find(l=>dd > -l);
    cls="wait"; signal="⚪ WAIT"; reason=next ? `Следващо наблюдавано ниво: -${next}%.` : "Няма активен входен сигнал.";
  }
  const div=document.createElement("article");
  div.className=`card ${cls}`;
  div.innerHTML=`
    <div class="top"><span class="symbol">${x.symbol}</span>
      <button class="remove" data-remove="${x.symbol}">×</button></div>
