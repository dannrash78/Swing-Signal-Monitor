...
renderMarketRegime();
renderCards();
+num(x.price)+'</b></td><td>'+actionText+'</td><td>'+trendText+'</td><td>'+escapeHtml(posText)+'</td><td>'+escapeHtml(buyText)+'</td><td><td>'+Number(x.drawdownPct).toFixed(2)+'%</td><td>'+(x.changePct>=0?"+":"")+Number(x.changePct).toFixed(2)+'%</td><td>'+escapeHtml(x.date||"—")+'</td><td>'+escapeHtml(finvizSummary(symbol))+'</td><td>'+escapeHtml(providerName(x.source))+'</td><td>'+(ownedNow?'<button type="button" class="sell-btn" data-sell="'+escapeHtml(symbol)+'">Продай</button>':'<button type="button" class="hide-btn" data-hide="'+escapeHtml(symbol)+'">Скрий</button>')+'</td>';
    bindTableRow(row,symbol);tbody.appendChild(row);
  });
  table.appendChild(tbody);wrap.appendChild(table);section.appendChild(wrap);
  section.querySelectorAll("[data-sell]").forEach(btn=>btn.onclick=()=>sellPosition(btn.dataset.sell));
...
renderMarketRegime();
renderCards();
+(Number.isFinite(Number(x.sma200))?num(x.sma200):"—")+'</td><td>'+rrText+'</td><td><td>'+Number(x.drawdownPct).toFixed(2)+'%</td><td>'+(x.changePct>=0?"+":"")+Number(x.changePct).toFixed(2)+'%</td><td>'+escapeHtml(x.date||"—")+'</td><td>'+escapeHtml(finvizSummary(symbol))+'</td><td>'+escapeHtml(providerName(x.source))+'</td><td>'+(ownedNow?'<button type="button" class="sell-btn" data-sell="'+escapeHtml(symbol)+'">Продай</button>':'<button type="button" class="hide-btn" data-hide="'+escapeHtml(symbol)+'">Скрий</button>')+'</td>';
    bindTableRow(row,symbol);tbody.appendChild(row);
  });
  table.appendChild(tbody);wrap.appendChild(table);section.appendChild(wrap);
  section.querySelectorAll("[data-sell]").forEach(btn=>btn.onclick=()=>sellPosition(btn.dataset.sell));
