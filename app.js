...
function renderTableGroup(section,groupSymbols,key){
  ...
  table.innerHTML='<thead><tr><th>Update</th><th>Акция</th><th>Price</th><th>Действие</th><th>Позиция</th><th>Допокупка</th><th>SMA20</th><th>SMA50</th><th>SMA200</th><th>60d high</th><th>От връха</th><th>Ден</th><th>Обновено</th><th>Finviz</th><th>Data</th><th></th></tr></thead>';
  ...
  groupSymbols.forEach(symbol=>{
    ...
    const buy=additionalBuyState(x,levels),entries=positionEntries(symbol),avg=weightedAverageEntry(entries),pnl=avg!==null?(x.price/avg-1)*100:null; const action=stockAction(x,target,levels),trend=stockTrend(x);
...
function stockAction(x,target,levels){
  ...
  if(entries.length){
    ...
    const pnl=avg!==null?(Number(x.price)/avg-1)*100:null;
    if(Number.isFinite(pnl)&&pnl>=target){
      return {kind:"exit",label:"REVIEW EXIT",reason:"Постигната е зададената цел +"+target+"%. Провери R/R, тренда и пазарния режим преди действие."};
    }
    if(market==="risk-off"&&trend.kind==="bear"){
      return {kind:"risk",label:"REVIEW RISK",reason:"Позицията е в пазарен Risk-Off и технически слаб trend. Следи риска, не само P/L."};
    }
    return {kind:"hold",label:"HOLD / MANAGE",reason:"Позицията остава под целта. Управлявай според текущия trend, market regime и риска."};
  }

  if(market==="unknown"){
    return {kind:"info",label:"UPDATE MARKET",reason:"Преди действие зареди Market Regime за SPY/QQQ."};
  }

  if(buy.kind==="entry"){
    if(market==="risk-on"&&trend.kind==="bull"){
      return {kind:"entry",label:"ENTRY REVIEW",reason:"Достигнато е drawdown ниво при Risk-On и bullish trend. Това е setup за преглед, не автоматична покупка."};
    }
    if(market==="risk-off"){
      return {kind:"wait",label:"WAIT / REVIEW",reason:"Достигнато е drawdown ниво, но широкият пазар е Risk-Off. Изчакай потвърждение и прецени риска."};
    }
    return {kind:"watch",label:"WATCH / REVIEW",reason:"Има ENTRY ZONE, но market/trend не са едновременно потвърдени."};
  }

  if(buy.level!==null){
    return {kind:"watch",label:"WATCH",reason:"Следващото зададено drawdown ниво е -"+buy.level+"%."};
  }
...
renderMarketRegime();
renderCards();
+num(x.price)+'</b></td><td><b>'+escapeHtml(action.label)+'</b><div class="table-company">'+escapeHtml(action.reason)+'</div></td><td>'+escapeHtml(posText)+'</td><td>'+escapeHtml(buyText)+'</td><td><td>'+Number(x.drawdownPct).toFixed(2)+'%</td><td>'+(x.changePct>=0?"+":"")+Number(x.changePct).toFixed(2)+'%</td><td>'+escapeHtml(x.date||"—")+'</td><td>'+escapeHtml(finvizSummary(symbol))+'</td><td>'+escapeHtml(providerName(x.source))+'</td><td>'+(ownedNow?'<button type="button" class="sell-btn" data-sell="'+escapeHtml(symbol)+'">Продай</button>':'<button type="button" class="hide-btn" data-hide="'+escapeHtml(symbol)+'">Скрий</button>')+'</td>';
    bindTableRow(row,symbol);tbody.appendChild(row);
  });
  table.appendChild(tbody);wrap.appendChild(table);section.appendChild(wrap);
  section.querySelectorAll("[data-sell]").forEach(btn=>btn.onclick=()=>sellPosition(btn.dataset.sell));
...
renderMarketRegime();
renderCards();
+(Number.isFinite(Number(x.sma20))?num(x.sma20):"—")+'</td><td><td>'+Number(x.drawdownPct).toFixed(2)+'%</td><td>'+(x.changePct>=0?"+":"")+Number(x.changePct).toFixed(2)+'%</td><td>'+escapeHtml(x.date||"—")+'</td><td>'+escapeHtml(finvizSummary(symbol))+'</td><td>'+escapeHtml(providerName(x.source))+'</td><td>'+(ownedNow?'<button type="button" class="sell-btn" data-sell="'+escapeHtml(symbol)+'">Продай</button>':'<button type="button" class="hide-btn" data-hide="'+escapeHtml(symbol)+'">Скрий</button>')+'</td>';
    bindTableRow(row,symbol);tbody.appendChild(row);
  });
  table.appendChild(tbody);wrap.appendChild(table);section.appendChild(wrap);
  section.querySelectorAll("[data-sell]").forEach(btn=>btn.onclick=()=>sellPosition(btn.dataset.sell));
...
renderMarketRegime();
renderCards();
+(Number.isFinite(Number(x.sma50))?num(x.sma50):"—")+'</td><td><td>'+Number(x.drawdownPct).toFixed(2)+'%</td><td>'+(x.changePct>=0?"+":"")+Number(x.changePct).toFixed(2)+'%</td><td>'+escapeHtml(x.date||"—")+'</td><td>'+escapeHtml(finvizSummary(symbol))+'</td><td>'+escapeHtml(providerName(x.source))+'</td><td>'+(ownedNow?'<button type="button" class="sell-btn" data-sell="'+escapeHtml(symbol)+'">Продай</button>':'<button type="button" class="hide-btn" data-hide="'+escapeHtml(symbol)+'">Скрий</button>')+'</td>';
    bindTableRow(row,symbol);tbody.appendChild(row);
  });
  table.appendChild(tbody);wrap.appendChild(table);section.appendChild(wrap);
  section.querySelectorAll("[data-sell]").forEach(btn=>btn.onclick=()=>sellPosition(btn.dataset.sell));
...
renderMarketRegime();
renderCards();
+(Number.isFinite(Number(x.sma200))?num(x.sma200):"—")+'</td><td><td>'+Number(x.drawdownPct).toFixed(2)+'%</td><td>'+(x.changePct>=0?"+":"")+Number(x.changePct).toFixed(2)+'%</td><td>'+escapeHtml(x.date||"—")+'</td><td>'+escapeHtml(finvizSummary(symbol))+'</td><td>'+escapeHtml(providerName(x.source))+'</td><td>'+(ownedNow?'<button type="button" class="sell-btn" data-sell="'+escapeHtml(symbol)+'">Продай</button>':'<button type="button" class="hide-btn" data-hide="'+escapeHtml(symbol)+'">Скрий</button>')+'</td>';
    bindTableRow(row,symbol);tbody.appendChild(row);
  });
  table.appendChild(tbody);wrap.appendChild(table);section.appendChild(wrap);
  section.querySelectorAll("[data-sell]").forEach(btn=>btn.onclick=()=>sellPosition(btn.dataset.sell));
...
renderMarketRegime();
renderCards();
+num(x.high60)+'</td><td>'+Number(x.drawdownPct).toFixed(2)+'%</td><td>'+(x.changePct>=0?"+":"")+Number(x.changePct).toFixed(2)+'%</td><td>'+escapeHtml(x.date||"—")+'</td><td>'+escapeHtml(finvizSummary(symbol))+'</td><td>'+escapeHtml(providerName(x.source))+'</td><td>'+(ownedNow?'<button type="button" class="sell-btn" data-sell="'+escapeHtml(symbol)+'">Продай</button>':'<button type="button" class="hide-btn" data-hide="'+escapeHtml(symbol)+'">Скрий</button>')+'</td>';
    bindTableRow(row,symbol);tbody.appendChild(row);
  });
  table.appendChild(tbody);wrap.appendChild(table);section.appendChild(wrap);
  section.querySelectorAll("[data-sell]").forEach(btn=>btn.onclick=()=>sellPosition(btn.dataset.sell));
